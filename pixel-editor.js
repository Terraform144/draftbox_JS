const PixelEditor = {
  ...
  bindEvents() {
    ...
    document.getElementById('saveSpriteBtn').addEventListener('click', () => self.saveSprite());
    document.getElementById('exportSpriteBtn').addEventListener('click', () => self.exportSprite());
    ...
    document.getElementById('showGrid').addEventListener('change', () => self.draw());

    this.canvas.addEventListener('mousedown', (e) => {
      if (e.button === 2) {
        const { x, y } = self.getPixelCoords(e);
        const color = self.pixels[y] && self.pixels[y][x];
        if (color) { self.currentColor = color; document.getElementById('colorPicker').value = color; }
        return;
      }
      const { x, y } = self.getPixelCoords(e);
  ...
  updateToolButtons() {
    ...
    const btn = document.getElementById(btnMap[this.currentTool]);
